...
namespace {
Vector2 CalculateCollisionPosition(int x, int y, const TileSet* tileset, int localId, const TMJMap& map) {
    Vector2 position = {static_cast<float>(x * map.tileWidth), static_cast<float>(y * map.tileHeight)};

    float offsetY = 0.0f;
    if (!tileset->isAtlas) {
        auto it = tileset->tileImages.find(localId);
        if (it != tileset->tileImages.end() && it->second && it->second->id != 0) {
            offsetY = static_cast<float>(map.tileHeight - it->second->height);
        }
    }
...
bool CheckCollisionWithShape(const Rectangle& rect, const PositionedCollision& collision) {
...
std::vector<PositionedCollision> GenerateCollisions(const TMJMap& map) {
    ...
    collisions.reserve(static_cast<size_t>(map.width * map.height));
